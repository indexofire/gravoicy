...
from category.managers import TopicManager()
from category.signals import *
...
class AbstractTopic(models.Model):
    ...
    def merge_to(self, parent):
        ...
        try:
            ...
            pre_move.send(sender=self, moving=[(self, parent)])
            self.move_to(parent)
            return
        ...
        if squash:
            pre_merge.send(sender=self, merge_pairs=squash)

        if to_move:
            pre_move.send(sender=self, moving=to_move)
...
class Topic(Base, AbstractTopic):
    """
...
if mptt_register:
    mptt_register.register(Topic, order_insertion_by=["name"])
